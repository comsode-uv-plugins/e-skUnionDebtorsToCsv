...
@DPU.AsExtractor
public class UnionFODlznici extends AbstractDpu<UnionFODlzniciConfig_V1> {
	private static final Logger LOG = LoggerFactory
			.getLogger(UnionFODlznici.class);
	...
	public UnionFODlznici() {
		super(UnionFODlzniciVaadinDialog.class, ConfigHistory
				.noHistory(UnionFODlzniciConfig_V1.class));
	}

	@Override
	protected void innerExecute() throws DPUException {
		File outputDirectory = null;
        String outputVirtualPath = "union_fo_dlznici.csv";
		...
		try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
			...
			for (Element el : rootDoc.select("a[href]")) {
				if (el.attr("href").endsWith("Zoznam_dlznikov_FO")) {
